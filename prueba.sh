...
if [[ -z $mask ]]; then
    if [[ -n $script ]]; then
        if [[ -n $passwdFile ]]; then
            ...
            echo ejecutar_script -p $pass $script $user $IP
            unset pass
        else
            leer_passwd
            echo ejecutar_script -p $SECRET_PASSWD $script $user $IP
        fi
    fi
    if [[ -n $comando ]]; then
        if [[ -n $passwdFile ]]; then
            pass=$(ccrypt -c -k key pass.conf | grep $IP | awk '{print $NF}' | sed -e 's/\[//; s/\]//')
            echo ejecutar_comando -p $pass $user $IP $comando
            unset pass
        else
            leer_passwd
            echo ejecutar_comando -p $SECRET_PASSWD $user $IP $comando
        fi
    fi
else
    ...
    for ((m = 1; m < $((${#networkIP[@]} - 1)); m++)); do
        ...
        if [[ -n $script ]]; then
            if [[ -n $passwdFile ]]; then
                ...
                echo ejecutar_script -p $pass $script $user ${networkIP[$m]}
                unset pass
            else
                leer_passwd
                echo ejecutar_script -p $SECRET_PASSWD $script $user ${networkIP[$m]}
            fi
        fi
        if [[ -n $comando ]]; then
            if [[ -n $passwdFile ]]; then
                pass=$(ccrypt -c -k key pass.conf | grep ${networkIP[$m]} | awk '{print $NF}' | sed -e 's/\[//; s/\]//')
                echo ejecutar_comando -p $pass $user ${networkIP[$m]} $comando
                unset pass
            else
                leer_passwd
                echo ejecutar_comando -p $SECRET_PASSWD $user ${networkIP[$m]} $comando
            fi
        fi
    done
