...
if [[ -n $script ]]; then
    if [[ -n $passwdFile ]]; then
        pass=`ccrypt -c -k key pass.conf | grep 10.0.0.176 | awk '{print $NF}' | sed -e 's/\[//; s/\]//'`
        ejecutar_script -p $pass
        unset pass
    else
        leer_passwd
        ejecutar_script $SECRET_PASSWD
    fi    
fi
if [[ -n $comando ]]; then
    if [[ -n $passwdFile ]]; then
        ejecutar_comando $passwdFile
    else
        leer_passwd
        ejecutar_comando $SECRET_PASSWD
    fi    
fi
